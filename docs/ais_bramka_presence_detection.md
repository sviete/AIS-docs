...
Wykrywanie obecności ma na celu ustalenie, czy i kto jest w domu (lub innej zdefiniowanej strefie), co jest bardzo cennym źródłem automatyzacji. Wiedza o tym, kto jest w domu lub gdzie jest, pozwala nam na całą gamę opcji automatyzacji, przykładowe przypadku użycia:
- Wyślij mi powiadomienie, gdy moje dziecko przybędzie do szkoły
- Włącz klimatyzację w domu, kiedy wychodzę z pracy
- Otwórz bramę i włącz na 30 minut oświetlenie zwnętrzne, jeżeli jest noc i moje auto pojawia się w strefie przed bramą wjazdowa

![Dodanie strefy](/AIS-docs/img/en/bramka/presence_detection_14.png)
...
> Asystent domowy będzie nasłuchiwać komunikatów o Twojej lokalizacji przychodzących z OwnTracks przez HTTPS. Żeby odbierać takie komunikaty nawet gdy jesteś poza lokalną domową siecią należy [włączyć dostęp  do bramki z Internetu](/AIS-docs/docs/en/next/ais_bramka_remote_dom_tunnel.html#włączenie-dostępu).

### AIS Wykrywanie obecności

Aby skonfigurować OwnTracks, przejdz do konfiguracji a następnie do panelu integracji. Naciśnij pomarańczowy przycisk z ikonką "plus" w prawym dolnym rogu ekranu, żeby otworzyć listę dostępnych integracji. Wybierz z listy integrację **AIS Wykrywanie obecności**
...
Aby dodać nową osobę, przejdz do konfiguracji a następnie wybierz opcję **Osoby**.
...
Możesz dodać zdjęcie osoby do wyświetlania na mapie w tym celu przejdz do **Konfiguracja** -> **Dostosowywanie**, wybierz osobę a następnie dodaj jej nowy atrybut **entity_picture** z adresem jej zdjęcia.
...
Aby skonfigurować nową stefę, przejdz do konfiguracji a następnie do panelu integracji. Naciśnij pomarańczowy przycisk z ikonką "plus" w prawym dolnym rogu ekranu, żeby otworzyć listę dostępnych integracji. Wybierz z listy integrację **Strefy**
...
Wyzwalacze strefy oraz geolokalizacji mogą uruchamiać się, gdy osoba wchodzi w strefę lub gdy opuszcza strefę.
W celu dodania automatyzacji opartej na lokalizacji, podczas dodawania automatyzacji wybierz typ wyzwalacza **Strefa** następnie wybierz osobę której dotyczy sprawdzenie lokalizacji
